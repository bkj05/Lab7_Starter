describe('Basic user flow for Website', () => {
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    const buttonTexts = await page.$$eval('product-item', products => {
      return products.map(product => product.shadowRoot.querySelector('button').innerText);
    });

    let allRemoved = true;

    for(let i = 0; i < buttonTexts.length; i++) {
      if(buttonTexts[i] !== 'Remove from Cart') {
        allRemoved = false;
      }
    }
